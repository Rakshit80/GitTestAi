...
import { CheckCircle2, Link2Icon, ListChecks, Loader2, Settings2, Sparkles, TrendingUp, XCircle } from 'lucide-react'
...
function UserRepoList({repoList,setReload}:props) {
  ...
  <AccordionItem key={index} value={(repo.repoId).toString()} className='border px-5 rounded-xl'>
    <AccordionTrigger>
        <div className='flex items-center gap-6'>
            <Image src={'/github.png'} alt= 'github' width={30} height={30}/>
            <div className='flex flex-col items-start gap-1'>
                <h2>{repo.fullName}</h2>
                <p className='text-sm text-gray-500'>{repo.defaultBranch}. {repo.language}</p>
            </div>
        </div> 
    </AccordionTrigger>
