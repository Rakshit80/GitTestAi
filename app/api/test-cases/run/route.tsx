...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const userIdNum = parseInt(testCase.userId);
    if (isNaN(userIdNum)) {
      return NextResponse.json({ error: "Invalid user ID on test case" }, { status: 400 });
    }
    ...
    if (!userRecord) {
      return NextResponse.json({ error: "User not found associated with this test case" }, { status: 404 });
    }

    const isRegenerating = mode === "generate" || !testCase.browserbaseScript;
    const cost = isRegenerating ? 10 : 5;

    if (userRecord.credits < cost) {
      return NextResponse.json(
        { error: `Insufficient credits. This test run requires ${cost} credits, but you only have ${userRecord.credits}.` },
        { status: 402 }
      );
    }

    // Deduct credits
    const updatedCredits = userRecord.credits - cost;
    await db
      .update(users)
      .set({ credits: updatedCredits })
      .where(eq(users.id, userRecord.id));

    // Save state for potential refund in outer catch block
    userRecordObj = userRecord;
    costCharged = cost;
    // ------------------------------
...
\`await page.goto('${baseUrl}${testCase.targetRoute || ""}', { waitUntil: 'load', timeout: 15000 })\` followed by a short settle wait: \`await page.waitForTimeout(1000)\`.
...
`;
    ...
    //   const connectUrl = bb.getConnectURL({ sessionId: session.id });
      ...
      return NextResponse.json({
        ...
        logs,
        browserbaseScript: scriptText,
        credits: userRecordObj ? userRecordObj.credits - costCharged : undefined,
      });
    } catch (execError: any) {
    ...
    if (userRecordObj && costCharged > 0) {
      try {
        await db
          .update(users)
          .set({ credits: userRecordObj.credits })
          .where(eq(users.id, userRecordObj.id));
      } catch (refundErr) {
