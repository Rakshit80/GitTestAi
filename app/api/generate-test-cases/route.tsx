...
async function getRepoTree({
  ...
  // 🔥 STEP 1: Get repo info → correct default branch
  ...
  if (!repoRes.ok) {
    ...
    console.error("REPO ERROR:", repoRes.status, err);
    throw new Error(`Failed to fetch repo info: ${repoRes.statusText}`);
  }

  const repoData = await repoRes.json();
  const actualBranch = repoData.default_branch; // ✅ always correct

  // 🔥 STEP 2: Get branch → SHA
  const branchRes = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/branches/${actualBranch}`,
    {
      headers: {
        Authorization: `Bearer ${githubToken}`,
        Accept: "application/vnd.github+json",
  ...
  if (!branchRes.ok) {
    ...
    console.error("BRANCH ERROR:", branchRes.status, err);
  ...
  // 🔥 STEP 3: Fetch repo tree using SHA
  ...
  if (!res.ok) {
    ...
    console.error("TREE ERROR:", res.status, err);
    throw new Error(`Failed to fetch GitHub repo tree: ${res.statusText}`);
  }

  const data = await res.json();
  const allBlobs = data.tree.filter((item: any) => item.type === "blob");
  
  let usefulFiles = allBlobs.filter((item: any) => isUsefulFile(item.path));
  
  // Fallback for simple repos (like competitive programming repos) with no standard extensions
  if (usefulFiles.length === 0) {
    usefulFiles = allBlobs;
  }
...
- targetRoute: most likely app route/page to test
